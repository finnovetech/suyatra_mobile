...
class _ArticleDetailsPageState extends State<ArticleDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: locator<NavigationService>().navigationKey.currentState!.canPop() ? true : false,
      onPopInvoked: (hasPopped) {
        if(hasPopped) {
          return;
        }
        if(!(locator<NavigationService>().navigationKey.currentState!.canPop())) {
  ...
  PreferredSizeWidget _appBar() {
    return AppBar(
      leading: locator<NavigationService>().navigationKey.currentState!.canPop() ? null : IconButton(
        onPressed: () {
          locator<NavigationService>().navigateTo(homeRoute);
        }, 
      ...
      actions: [
        IconButton(
          onPressed: () async {
            Share.share("$websiteUrl/stories/${widget.article.slug}");
          },
              ...
              children: [
                Hero(
                  ...
                  child: CachedImageWidget(
                    opacity: 0.7,
                    imageUrl: "$websiteUrl${widget.article.image}",
                    fit: BoxFit.cover,
                    ...
                    children: [
                      Text(
                        state.articleCategories!
                            .where((element) =>
                                element.id == widget.article.categories?.firstOrNull)
                            .firstOrNull
                            ?.title
                            .toUpperCase() ?? "",
                ...
                _commentSection(state),
                const SizedBox(height: 32.0),
                _relatedArticlesList(state),
              ],
            ),
          );
        },
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      (locator<FirebaseService>().firebaseAuth.currentUser?.email?.characters.firstOrNull ?? "G").toUpperCase(),
                      style: const TextStyle(
                        fontSize: h7,
                        fontWeight: FontWeight.w600,
                      ),
                ...
                InkWell(
                  onTap: () {
                    context.read<ArticleCubit>().addArticleComment(
                      article: widget.article,
                      comment: {
                        "data": state.commentContoller!.text,
                        "user_name": locator<FirebaseService>().firebaseAuth.currentUser?.email ?? "",
                      }
                    );
                  },
              ...
              ? CarouselSlider.builder(
                  ...
                  itemBuilder: (context, index, _) {
                    ...
                    return Container(
                      ...
                      margin: EdgeInsets.only(right: index == state.articleComments!.length - 1 ? 0: 16.0).copyWith(top: 12.0),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Row(
